...
#drive_auth_configure(api_key = read.csv("keys/ket.txt",header=FALSE,stringsAsFactors = FALSE)[,1])
drive_auth(path="keys/alien-craft-256317-031817639c3a.json")
...
Terms<-distinct(data[c("uri","Term","Source")])%>%arrange(Term)
Terms$Term<-paste0(Terms$Term," (",Terms$Source,")")
data$Source <- paste0('<a href=', data$voclink,' target="_blank" rel="noopener"','>',data$Source,'</a>')
data$Term <- paste0(data$Term,'<a href=', data$uri,' target="_blank" rel="noopener"','> ',as.character(icon("share", lib = "glyphicon")),'</a>')
data<-data[c("uri","Term","Definition","Source","voclink","Organization","Theme")]
...
column(1,pickerInput(
 ...
 #  label = "Organizations(s)",
   ...
   multiple=TRUE, width=300, options = list(
      `actions-box` = TRUE, `selected-text-format`='static',`title`="Select Source(s)")
),offset=3))
