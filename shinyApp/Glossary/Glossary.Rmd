...
Step 1: Select term to for which to identify synonyms
...
Step 2. Click all rows corresponding to terms you believe are synonymous with the term selected in Step 1. The glossary has provided the 50 terms with the most commonality in the word content of definitions ("score")  to the selected term.
```{r}
...
sel_uris<-reactive({SPARQL("http://35.238.222.96:7200/repositories/Synonym_search",sel_parse(),format="csv")$result%>%mutate(score=round(score,2))})
 ...
 output$seltable = DT::renderDataTable({
   DT::datatable(sel_uris(),options = list(
     dom='ti',"pageLength" = 5000,scrollY='55vh'), rownames=FALSE, escape=FALSE, selection="multiple"
   )
 })
...
sel_syn = reactive({sel_uris()[input$seltable_rows_selected,]})
 ...
 output$selsyn = DT::renderDataTable({
   DT::datatable(sel_syn()[c("Term","Definition","Vocabulary")], extensions = c('Buttons','Responsive'),options = list(
     dom='Bt', buttons=c('csv'),"pageLength" = 5000,scrollY='70vh'), rownames=FALSE, escape=FALSE, selection="single"
   )
 })
