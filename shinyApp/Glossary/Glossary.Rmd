...
### Table of terms: Click rows to view synonyms and related terms on the right
...
column(1,pickerInput(
   inputId = "filter2",
   label = "Filter by Organizations(s)", 
...
output$table = DT::renderDataTable({
  DT::datatable(d(), options = list(columnDefs = list(list(targets = c(0,3,4,6), visible = FALSE)),
    dom='Bfti', buttons=c('csv','excel','copy'),scrollY='60vh',"pageLength" = 5000), rownames=FALSE, escape=FALSE, selection="single", extensions = c('Buttons')
  )
})
...
# verbatimTextOutput('x4')
 ...
 output$syntable = DT::renderDataTable({
   DT::datatable(syn_d()[c("Term","Definition","Vocabulary")], extensions = c('Buttons'),options = list(
     dom='Bfti', buttons=c('csv','excel','copy'),scrollY='30vh',"pageLength" = 5000), rownames=FALSE, escape=FALSE, selection="single"
   )
 })
...
related_group=reactive({as.numeric(syn_groups$group[which(rel_groups$uri==r())])})
...
# verbatimTextOutput('x4')
 ...
 output$reltable = DT::renderDataTable({
   DT::datatable(rel_d()[c("Term","Definition","Vocabulary")], extensions = c('Buttons'),options = list(
     dom='Bfti', buttons=c('csv','excel','copy'),scrollY='30vh',"pageLength" = 5000), rownames=FALSE, escape=FALSE, selection="single"
   )
 })
