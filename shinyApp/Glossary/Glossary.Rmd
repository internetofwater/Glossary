...
endpoint<-"http://35.238.222.96:7200/repositories/Synonym_search"
...
Terms<-distinct(select(data,uri,Term,Source))%>%as.data.frame
...
```{r}
 ...
 output$orgtable = DT::renderDataTable({
   DT::datatable(orgs, extensions = c('Buttons','Responsive'),options = list(dom='Bit', buttons = list(list(extend = 'csv',text="Download CSV", filename= paste0('vocabularies'))),"pageLength" = 5000,scrollY='55vh'), rownames=FALSE, escape=FALSE, selection="single"
   )
 })
...
data<-mutate(data,Theme = ifelse(!is.na(Theme), Theme, "Other"))%>% as.data.frame()

#data$Theme[which(is.na(data$Theme))]<-"Other"
#data2<-as.data.frame(data)
d <- reactive({
  #data <- data[Organization %in% input$filter2 & Theme %in% input$filter]
  data%>%filter(Theme %in% input$filter & Organization %in% input$filter2  )%>%distinct(uri,.keep_all=T)
})
...
<h5>Step 2. Click all rows corresponding to concepts you believe are synonymous with the concepts selected in Step 1.</h5> The glossary has provided the 50 terms with the most commonality in the word content of definitions ("score")  to the selected concept, as well as all other concepts in the glossary associated with the same term (i.e. homonyms). Synonyms that have already been identified and approved are not shown.
```{r}
...
#name<-reactive({as.character(paste0("Glossary_Resources/Synonym_Proposal_Sheets/",selected_uri(),"_",".csv"))})
 ...
 observeEvent(input$do2, {
  ...
  upload <- drive_upload("synonyms.csv",paste0("Glossary_Resources/Synonym_Proposal_Sheets/",as.character(Terms$Term[which(Terms$Term == input$search)]),"-",substr(as.character(Terms$uri[which(Terms$Term == input$search)]),30,100),"-",input$emailInput,"-synonyms_submit"),type="spreadsheet")
    ...
    sendSweetAlert(session=session,title="Synonyms proposed!", text="Thank you for your contribution! Press 'ok' to continue browsing the glossary. Feel free to submit synonym proposals for more concepts.",type="success")
