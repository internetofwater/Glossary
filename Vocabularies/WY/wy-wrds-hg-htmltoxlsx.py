...
result = requests.get("http://library.wrds.uwyo.edu/glossary/wrs/wrs01/wrs01-ht.html")
...
############################################################################
    ...
    for i in lst1:
        if '* * *' in i:  #WY glossary specific
            continue
        if ' - ' in i:    #WY glossary specific
            lst2.append(i.split(' - ')[0])  #index 0 is the term name
    ...
    with pd.ExcelWriter('Wyoming Hydrologic Terms and Defs.xlsx') as writer:  #Name of xlsx file.
